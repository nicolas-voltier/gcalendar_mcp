...
# Configure logging
# Use the current working directory for logs
log_dir = Path.cwd()
log_file = log_dir / "calendar_mcp_server.log"
...
# Use the current working directory for tokens and credentials
TOKEN_DIR = Path.cwd()
...
def get_credentials():
    ...
    if TOKEN_FILE.exists():
        logger.info(f"Found existing token file at {TOKEN_FILE}")
